...
def resolve_feature2_path(job):
    output_feature_file = Path(clean_text(job.get("output_feature_file", "")))

    candidates = [
        output_feature_file.parent / "Feature2.csv",
        output_feature_file.with_name("Feature2.csv"),
    ]
...
def combine_feature2(jobs_file, output_file):
    ...
    for job in jobs:
        feature2_path = resolve_feature2_path(job)

        if not feature2_path.exists():
            missing_files += 1
            continue

        try:
            rows = read_csv_rows(feature2_path)
            ...
            row = rows[0]
            row["feature2_file"] = str(feature2_path)
    ...
    print(f"[MISSING FEATURE2 FILES] {missing_files}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine all per-GWAS Feature2.csv files into one Variation2_Allfeatures.csv file."
    )
    ...
    parser.add_argument(
        "--output-file",
        default="Feature2_All.csv",
        help="Combined output CSV. Default: Feature2_All.csv",
    )
