...
def resolve_feature3_path(job):
    output_feature_file = Path(clean_text(job.get("output_feature_file", "")))

    candidates = [
        output_feature_file.parent / "Feature3.csv",
        output_feature_file.with_name("Feature3.csv"),
    ]
...
def combine_feature3(jobs_file, output_file):
    ...
    for job in jobs:
        feature3_path = resolve_feature3_path(job)

        if not feature3_path.exists():
            missing_files += 1
            continue

        try:
            rows = read_csv_rows(feature3_path)
            ...
            row = rows[0]
            row["feature3_file"] = str(feature3_path)
    ...
    print(f"[MISSING FEATURE3 FILES] {missing_files}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine all per-GWAS Feature3.csv files into one Feature3_All.csv file."
    )
    ...
    parser.add_argument(
        "--output-file",
        default="Feature3_All.csv",
        help="Combined output CSV. Default: Feature3_All.csv",
    )
