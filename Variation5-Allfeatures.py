...
def resolve_feature5_path(job):
    output_feature_file = Path(clean_text(job.get("output_feature_file", "")))

    candidates = [
        output_feature_file.parent / "Feature5.csv",
        output_feature_file.with_name("Feature5.csv"),
    ]
...
def combine_feature5(jobs_file, output_file):
    ...
    for job in jobs:
        feature5_path = resolve_feature5_path(job)

        if not feature5_path.exists():
            missing_files += 1
            continue

        try:
            rows = read_csv_rows(feature5_path)
            ...
            row = rows[0]
            row["feature5_file"] = str(feature5_path)
    ...
    print(f"[MISSING FEATURE5 FILES] {missing_files}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine all per-GWAS Feature5.csv files into one Feature5_All.csv file."
    )
    ...
    parser.add_argument(
        "--output-file",
        default="Feature5_All.csv",
        help="Combined output CSV. Default: Feature5_All.csv",
    )
