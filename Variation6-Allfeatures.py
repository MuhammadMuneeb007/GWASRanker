...
def resolve_feature6_path(job):
    output_feature_file = Path(clean_text(job.get("output_feature_file", "")))

    candidates = [
        output_feature_file.parent / "Feature6.csv",
        output_feature_file.with_name("Feature6.csv"),
    ]
...
def combine_feature6(jobs_file, output_file):
    ...
    for job in jobs:
        feature6_path = resolve_feature6_path(job)

        if not feature6_path.exists():
            missing_files += 1
            continue

        try:
            rows = read_csv_rows(feature6_path)
            ...
            row = rows[0]
            row["feature6_file"] = str(feature6_path)
    ...
    print(f"[MISSING FEATURE6 FILES] {missing_files}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine all per-GWAS Feature6.csv files into one Feature6_All.csv file."
    )
    ...
    parser.add_argument(
        "--output-file",
        default="Feature6_All.csv",
        help="Combined output CSV. Default: Feature6_All.csv",
    )
