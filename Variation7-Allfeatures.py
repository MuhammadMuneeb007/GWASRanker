...
def resolve_feature7_path(job):
    output_feature_file = Path(clean_text(job.get("output_feature_file", "")))

    candidates = [
        output_feature_file.parent / "Feature7.csv",
        output_feature_file.with_name("Feature7.csv"),
    ]
...
def combine_feature7(jobs_file, output_file):
    ...
    for job in jobs:
        feature7_path = resolve_feature7_path(job)

        if not feature7_path.exists():
            missing_files += 1
            continue

        try:
            rows = read_csv_rows(feature7_path)
            ...
            row = rows[0]
            row["feature7_file"] = str(feature7_path)
    ...
    print(f"[MISSING FEATURE7 FILES] {missing_files}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine all per-GWAS Feature7.csv files into one Feature7_All.csv file."
    )
    ...
    parser.add_argument(
        "--output-file",
        default="Feature7_All.csv",
        help="Combined output CSV. Default: Feature7_All.csv",
    )
