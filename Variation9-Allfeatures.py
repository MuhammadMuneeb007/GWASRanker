...
def resolve_feature9_path(job):
    output_feature_file = Path(clean_text(job.get("output_feature_file", "")))
    file_path = Path(clean_text(job.get("file_path", "")))

    candidates = [
        output_feature_file.parent / "Feature9.csv",
        output_feature_file.with_name("Feature9.csv"),
        file_path.parent / "Feature9.csv",
    ]
...
def combine_feature9(jobs_file, output_file):
    ...
    for job in jobs:
        feature_path = resolve_feature9_path(job)
        ...
        try:
            ...
            row = rows[0]
            row["feature9_file"] = str(feature_path)
    ...
    print("=" * 100)
    print("[COMBINED Feature9.csv FILES]")
    print(f"[INPUT JOBS] {jobs_file}")
    print(f"[TOTAL JOBS] {len(jobs)}")
    print(f"[FILES COMBINED] {len(merged_rows)}")
    print(f"[MISSING FEATURE9 FILES] {missing_files}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine all per-GWAS Feature9.csv files into one Feature9_All.csv file."
    )
    parser.add_argument("--jobs-file", default="GWAS_jobs.csv")
    parser.add_argument("--output-file", default="Feature9_All.csv")
    args = parser.parse_args()
    combine_feature9(args.jobs_file, args.output_file)
